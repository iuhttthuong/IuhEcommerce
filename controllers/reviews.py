...
@router.post("/reviews", response_model=ReviewCreatePayload)
...
@router.get("/reviews/{review_id}", response_model=ReviewCreatePayload)
def get_review(review_id: int):
    try:
        ...
        raise HTTPException(status_code=500, detail=str(e))
@router.put("/reviews/{review_id}", response_model=ReviewUpdatePayload)    
def update_review(review_id: int, payload: ReviewUpdatePayload):
    try:
        ...
        raise HTTPException(status_code=500, detail=str(e))
@router.delete("/reviews/{review_id}", response_model=ReviewDeletePayload)
def delete_review(review_id: int):
    try:
        ...
        raise HTTPException(status_code=500, detail=str(e))
@router.get("/reviews/product/{product_id}", response_model=List[ReviewCreatePayload])
